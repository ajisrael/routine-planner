...
- Post-conversion polish (2026-09-06, user decisions via lavish artifact .lavish/setup-as-plan-view.html): (1) Setup chrome = slim DaisyUI steps row in the header (hub cards removed; steps clickable to jump, per-scale status as a warning badge only when incomplete, ✓ data-content for done steps). (2) Kept in setup scopes: person chips, conflict badge, category chips in rail. (3) Mode switcher hidden in setup stages; monthly stage shows it only inside a day drill (switcher IS the back-to-month path - user chose "switcher only" over a dedicated button while removing the always-visible switcher). (4) Add-task noise removed: QuickAddTask deleted; LibraryRail gets a "+ Add task" button that opens the shared TaskForm dialog with the stage's cadence preselected (TaskForm gained presetCadence). Verified E2E: steps jump, dialog opens daily-preset, "Stretch" created through it, monthly drill switcher present. Empty-state hints unchanged.
